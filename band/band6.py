...
class MiBand6(gatt.Device):
    ...
    def send_alert(self):
        ...
        print("Sending vibratory alert to driver!")
        self.charNotif.write_value([0x03, 0x01, 0x0a, 0x0a, 0x0a])

    def print_hr(self):
        
        """ Function to save recorded heart rate """

        self.hrHist[1] = self.hrHist[1][1:]
    ...
    def characteristic_value_updated(self, characteristic, value):
        ...
        if str(characteristic.uuid) == UUIDS.CHARACTERISTIC_CHUNKED_TRANSFER_READ:
            ...
            if hRate != 255 and hRate != 0:
                ...
                self.hrHist[1].append(time.time() - timer + self.hrHist[1][-1])
                timer = time.time()

                # Check if the heart rate is decreasing and send alert if needed
                if len(self.hrHist[0]) > 40 and hRate < np.mean(np.array(self.hrHist[0])) - 15:
                    self.send_alert()
